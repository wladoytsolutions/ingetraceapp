	...
	setTimeout(function () {
		...
		$("#btn_buscarGrafico").click(function(e) {
			...
			if(formularioBusqueda.checkValidity())
			{
				...
				var ValOp=ValidarFechasOperaciones($('#inicio_filtroSensorSoloPuerta').val(),$('#termino_filtroSensorSoloPuerta').val());
				if(ValOp=="ok"){
					//guardar_registro();
					$('#DivInicioOp').attr('class','form-group');
					$('#DivTerminoOp').attr('class','form-group');
				...
				}else
				{
					...
					}else
					{
						if(ValOp=="intervalo")
						{
							...
							MensajeAlerta('Error','La fecha de termino de operaciones('+$('#termino_filtroSensorSoloPuerta').val()+') es menor a la fecha de inicio de operaciones('+$('#inicio_filtroSensorSoloPuerta').val()+')');
						}
						if(ValOp=="exceso")
						{
							$('#DivInicioOp').attr('class','form-group has-error');
