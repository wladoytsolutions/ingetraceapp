	...
	$("#btn_buscarGrafico").prop('disabled', true);
		...
		$("#btn_buscarGrafico").click(function(e) {
			...
			if(formularioBusqueda.checkValidity())
			{
				...
				var ValOp=ValidarFechasOperaciones($('#inicio_filtroDatosSensor').val(),$('#termino_filtroDatosSensor').val());
				if(ValOp=="ok"){
					//guardar_registro();
					$('#DivInicioOp').attr('class','form-group');
					$('#DivTerminoOp').attr('class','form-group');
				...
				}else
				{
					if(ValOp=="intervalo")
					{
						...
						MensajeAlerta('Error','La fecha de termino de operaciones('+$('#termino_filtroDatosSensor').val()+') es menor a la fecha de inicio de operaciones('+$('#inicio_filtroDatosSensor').val()+')');
					}
					if(ValOp=="exceso")
					{
						$('#DivInicioOp').attr('class','form-group has-error');
